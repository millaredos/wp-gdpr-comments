...
class Millaredos_GDPR_Comments_ARCO_Portal {
	...
	private function render_user_data_panel( $email, $token ) {
		global $wpdb;
		$comments = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM %i WHERE comment_author_email = %s AND comment_approved != 'spam' ORDER BY comment_date DESC", $wpdb->comments, $email ) );
	...
	private function process_anonymization_request() {
		...
		// Log Right to Erasure with previous hash link
		MGC_Audit_Log::write( array(
			'comment_id'  => $comment_id,
			'change_type' => 'right_to_erasure',
			'user_id'     => 0,
			'old_value'   => $old_email,
