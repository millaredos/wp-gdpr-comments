...
class Millaredos_GDPR_Comments_Admin {
	...
	public function init() {
		...
		// Policy Versioning
		add_action( 'updated_option', array( $this, 'maybe_snapshot_policy' ), 10, 3 );

		// AJAX
		add_action( 'wp_ajax_mgc_view_policy', array( $this, 'ajax_view_policy' ) );
	}

	/**
	 * AJAX Callback to view policy version or audit log details.
	 */
	public function ajax_view_policy() {
		check_ajax_referer( 'mgc_audit_nonce', 'nonce' );

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( __( 'Insufficient permissions.', 'millaredos-gdpr-comments' ) );
		}

		$log_id = isset( $_GET['log_id'] ) ? absint( wp_unslash( $_GET['log_id'] ) ) : 0;
		$type   = isset( $_GET['type'] ) ? sanitize_key( wp_unslash( $_GET['type'] ) ) : 'audit';
		if ( ! $log_id ) {
			wp_send_json_error( __( 'Invalid ID.', 'millaredos-gdpr-comments' ) );
		}
		...
		if ( 'version' === $type ) {
			$table = $wpdb->prefix . 'mgc_policy_versions';
			$content = $wpdb->get_var( $wpdb->prepare( "SELECT content_json FROM %i WHERE id = %d", $table, $log_id ) );
		} else {
			$table = $wpdb->prefix . 'mgc_audit_log';
			$row = $wpdb->get_row( $wpdb->prepare( "SELECT old_value, new_value FROM %i WHERE id = %d", $table, $log_id ) );
			if ( $row ) {
		...
		if ( ! $content ) {
			wp_send_json_error( __( 'Content not found.', 'millaredos-gdpr-comments' ) );
		}

		// Check if it's JSON
		$decoded = json_decode( $content );
		if ( null !== $decoded ) {
			$content = wp_json_encode( $decoded, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE );
		}

		// Highlighting and escaping
		$escaped_content = esc_html( $content );
		$highlighted = preg_replace( '/&quot;([^&]+)&quot;:/', '<span class="mgc-json-key">"$1"</span>:', $escaped_content );

		$output = '<div class="mgc-json-viewer"><pre>' . $highlighted . '</pre></div>';

		// Use wp_kses to allow the formatted HTML while ensuring security.
		$allowed_tags = array(
		...
		wp_send_json_success( wp_kses( $output, $allowed_tags ) );
	}

	/**
	 * Create a new policy snapshot if any legal text has changed.
	 *
	 * @param string $option    Option name.
	 * @param mixed  $old_value Old value.
	 * @param mixed  $value     New value.
	 */
	public function maybe_snapshot_policy( $option, $old_value, $value ) {
		$watched_options = array(
			'mgc_responsible',
			'mgc_purpose',
			'mgc_legitimation',
			'mgc_hosting_provider',
			'mgc_recipients',
			'mgc_rights',
			'mgc_policy_link',
			'mgc_additional_info',
		...
		// Avoid duplicate snapshots in the same request
		static $snapshot_done = false;
		if ( true === $snapshot_done ) {
			return;
		}

		$current_policy = array();
		foreach ( $watched_options as $opt ) {
			$current_policy[ $opt ] = get_option( $opt );
		}

		$content_json = wp_json_encode( $current_policy );
		$version_hash = hash( 'sha256', $content_json );

		global $wpdb;
		$table_name = $wpdb->prefix . 'mgc_policy_versions';

		// Check if this exact version already exists
		$existing_id = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM %i WHERE version_hash = %s", $table_name, $version_hash ) );
		...
		if ( null === $existing_id ) {
			...
			update_option( 'mgc_active_policy_id', $new_id );
			...
			update_option( 'mgc_active_policy_id', $existing_id );
		}
	...
	public function enqueue_admin_styles() {
		...
		if ( 'millaredos-gdpr-comments' === $page ) {
			...
			// Admin JS
			wp_enqueue_script( 'mgc-admin-js', MGC_URL . 'assets/js/admin.js', array( 'jquery', 'thickbox' ), MGC_VERSION, true );
			wp_localize_script( 'mgc-admin-js', 'mgc_admin', array(
				'ajaxurl' => admin_url( 'admin-ajax.php' ),
				'nonce'   => wp_create_nonce( 'mgc_audit_nonce' ),
				'loading' => __( 'Loading...', 'millaredos-gdpr-comments' ),
				'error'   => __( 'An error occurred.', 'millaredos-gdpr-comments' ),
			) );
	...
	public function register_settings() {
		...
		register_setting( 'mgc_compliance_settings', 'mgc_arco_page_id', array( 'sanitize_callback' => 'absint' ) );
	...
	private function render_audit_filters_bar(  $filters  ) {
		...
		foreach ( $statuses as $key => $label ) {
			$class   = ( $key === $filters['status'] ) ? 'current' : '';
	...
	private function render_audit_table(  $comments  ) {
		...
		if ( ! empty( $comments ) ) {
			...
			foreach ( $comments as $comment ) {
				...
				$meta_policy_id = get_comment_meta( $comment->comment_ID, '_mgc_policy_version_id', true );
				...
				if ( ( $comment->comment_date < $install_date ) && ! $meta_hash ) {
					...
					echo esc_html( $meta_hash );
					
					if ( $meta_policy_id ) {
						echo '<br><a href="#" class="mgc-view-details button button-small" style="margin-top:5px;" data-id="' . absint( $meta_policy_id ) . '" data-type="version" title="' . esc_attr__( 'Accepted Policy Version', 'millaredos-gdpr-comments' ) . '"><span class="dashicons dashicons-visibility" style="font-size:14px; margin-top:4px;"></span> ' . sprintf( esc_html__( 'Policy v%d', 'millaredos-gdpr-comments' ), absint( $meta_policy_id ) ) . '</a>';
					}
	...
	public function render_logs_tab() {
		...
		if ( ! empty( $logs ) ) {
			foreach ( $logs as $log ) {
				...
				$user_display = $user ? $user->display_name : ( 0 === (int) $log->user_id ? __( 'System/Guest', 'millaredos-gdpr-comments' ) : sprintf( _x( '#%d', 'User ID prefix', 'millaredos-gdpr-comments' ), absint( $log->user_id ) ) );
				// Badge style for types
				$badge_class = 'mgc-badge';
				if ( in_array( $log->change_type, array( 'consent_given', 'policy_updated' ), true ) ) {
					$badge_class .= ' mgc-badge-success';
				} elseif ( false !== strpos( $log->change_type, 'admin_edit_' ) ) {
					$badge_class .= ' mgc-badge-warning';
				} elseif ( 'arco_erasure' === $log->change_type ) {
					$badge_class .= ' mgc-badge-danger';
				}
				...
				echo '<td>';
				echo '<div style="max-height: 60px; overflow-y: auto; font-size: 11px; margin-bottom: 5px;">' . esc_html( $log->new_value ) . '</div>';
				echo '<a href="#" class="mgc-view-details button button-small" data-id="' . absint( $log->id ) . '" data-type="audit" title="' . esc_attr__( 'Log Details', 'millaredos-gdpr-comments' ) . '">' . esc_html__( 'View Full Details', 'millaredos-gdpr-comments' ) . '</a>';
				echo '</td>';
				echo '</tr>';
			}
		} else {
	...
	private function translate_change_type( $type ) {
		$map = array(
			...
			'policy_updated'     => __( 'Policy Updated', 'millaredos-gdpr-comments' ),
			'arco_access'        => __( 'ARCO Access', 'millaredos-gdpr-comments' ),
			'arco_erasure'       => __( 'ARCO Erasure', 'millaredos-gdpr-comments' ),
			'admin_edit_content' => __( 'Admin Edit: Content', 'millaredos-gdpr-comments' ),
			'admin_edit_author'  => __( 'Admin Edit: Author', 'millaredos-gdpr-comments' ),
			'admin_edit_email'   => __( 'Admin Edit: Email', 'millaredos-gdpr-comments' ),
			'user_edit_content'  => __( 'User Edit: Content', 'millaredos-gdpr-comments' ),
			'user_edit_author'   => __( 'User Edit: Author', 'millaredos-gdpr-comments' ),
			'user_edit_email'    => __( 'User Edit: Email', 'millaredos-gdpr-comments' ),
		);
