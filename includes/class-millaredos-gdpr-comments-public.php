...
class Millaredos_GDPR_Comments_Public {
	...
	public function validate_gdpr_consent(  $commentdata  ) {
		// Verify nonce
		if ( ! isset( $_POST['mgc_comment_compliance_nonce'] ) || ! wp_verify_nonce( $_POST['mgc_comment_compliance_nonce'], 'mgc_comment_compliance_action' ) ) {
			wp_die(
		...
		// Verify Checkbox
		$consent = isset( $_POST['mgc_privacy_consent'] ) ? sanitize_text_field( $_POST['mgc_privacy_consent'] ) : '';
		if ( 'yes' !== $consent ) {
			wp_die(
	...
	public function save_consent_meta(  $comment_id  ) {
		...
		if ( 'yes' === $consent ) {
			...
			// 2. Generate Policy Text Hash (Snapshot of what was accepted)
			$policy_content = get_option( 'mgc_responsible', '' ) . '|' .
				get_option( 'mgc_purpose', '' ) . '|' .
				get_option( 'mgc_legitimation', '' ) . '|' .
				get_option( 'mgc_recipients', '' ) . '|' .
				get_option( 'mgc_rights', '' ) . '|' .
				get_option( 'mgc_policy_link', '' );
			$policy_hash    = hash( 'sha256', $policy_content );

			// 3. Generate Integrity Proof (HMAC)
			$secret         = wp_salt( 'nonce' );
			$data_to_sign   = $comment_id . '|' . $email . '|' . $ip_anon . '|' . $timestamp . '|' . $policy_hash;
			...
			add_comment_meta( $comment_id, '_mgc_policy_text_hash', $policy_hash ); // What did they accept?
			...
			$this->log_audit_event(
				...
				'consent_given',
				'',
				__( 'Consent given for Privacy Policy.', 'millaredos-gdpr-comments' )
			);
		}
	}
